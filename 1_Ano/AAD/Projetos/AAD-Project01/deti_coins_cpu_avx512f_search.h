...
static void deti_coins_cpu_avx512_search(void)
{
    ...
    u08_t *bytes;
    static u32_t interleaved_coins[13u * 16u] __attribute__((aligned(64))); // 16 DETI coins
    ...
    for (n_attempts = n_coins = 0ul; stop_request == 0; n_attempts++)
    {
        ...
        for(idx = 0;idx < 16;idx++){
            for(n = 0;n < 12;n++){
                interleaved_coins[16*n+idx] = coin[n];
            }
            interleaved_coins[16*n+idx] = coin[n] + idx; // change last block of the coin to introduce variability
        }

        //
        // compute MD5 hash using AVX-512
        //
        md5_cpu_avx512((v16si *)interleaved_coins, (v16si *)interleaved_hash);  
