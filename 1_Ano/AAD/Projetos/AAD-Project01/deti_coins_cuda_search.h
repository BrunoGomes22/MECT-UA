...
static void deti_coins_cuda_search(u32_t n_random_words)
{
    ...
    {
        host_data[0] = 1u; // index zero stores the first free position in the host_data array
    ...
    STORE_DETI_COINS();
    printf("deti_coins_cuda_search: %lu DETI coins%s found in %lu attempts (expected %.2f coins)\n",n_coins,(n_coins == 1ul) ? "" : "s",n_attempts,(double)n_attempts / (double)(1ul << 32));
